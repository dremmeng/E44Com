...
def phase_plan():
    """Print the intended production matrix for the H^1 program."""
    print("\nProduction H^1 run matrix\n")
    rows = [
        ("Full E(4,4)", "A/B", "[x]", "production_run.py build/cohomology"),
        ("Full E(4,4) + nu", "A/B", "[~]", "nu_deformation.py --cx A|B --step 5|6"),
        ("Borel reduction", "A/B", "[~]", "borel_chain_map_check.py"),
        ("Borel reduction + nu", "A/B", "[ ]", "not yet wired"),
    ]
    print(f"  {'Track':<24} {'Target':<8} {'Status':<6} Command / note")
    print("  " + "-" * 78)
    for track, target, status, command in rows:
        print(f"  {track:<24} {target:<8} {status:<6} {command}")
    print("\nRecommended order:")
    print("  1. Full E(4,4) A, then B")
    print("  2. Full E(4,4) + nu, A then B")
    print("  3. Borel reduction checks")
    print("  4. Borel + nu once the reduction is integrated")
...
def phase_build(cx_type, t_min, t_max, a_max, max_deg, e44_data):
    ...
    for idx, (k, k_tar) in enumerate(todo):
        ...
        print(f'[{ts()}]   -> {D.nrows()}times{D.ncols()}, {nnz} nnz, '
...
def cohomology_at_from_matrices(groups, differentials, k, t_min, t_max,
    ...
    print(f'  [k={k:+d}] rank of incoming {I_k.nrows()}times{I_k.ncols()} ...', flush=True)
    t0 = time.time()
    if I_k.ncols() == 0:
        dim_im = 0
    else:
...
def phase_cohomology(cx_type, t_min, t_max, a_max, max_deg, e44_data,
    ...
    for k in target_levels:
        ...
        d2_flag = '[OK]' if r['im_subset_ker'] else '[X]'
        print(f"  {k:>4}  {r['dim_Ck']:>9}  {r['dim_ker']:>9}  "
